...
def generate_static_mappings(
    xml_tag_mappings: Dict[str, str],
    class_import_paths: Dict[str, str],
    polymorphic_types: Dict[str, List[str]],
) -> str:
    """Generate static Python mappings module from YAML data.

    This function generates a Python module with hardcoded dictionaries
    for xml_tag_mappings, class_import_paths, and polymorphic_types.
    ...
    Args:
        xml_tag_mappings: Dictionary mapping XML tags to class names
        class_import_paths: Dictionary mapping class names to import paths
        polymorphic_types: Dictionary mapping base types to implementation lists

    Returns:
        Python code for static mappings module
    """
    import datetime

    # Generate header
    code = f'''"""Pre-compiled model mappings for fast deserialization.
...
'''
    ...
    # Generate XML_TAG_MAPPINGS dictionary
    code += "# XML tag to class name mappings\n"
    code += "XML_TAG_MAPPINGS: Dict[str, str] = {\n"
    for xml_tag, class_name in sorted(xml_tag_mappings.items()):
        # Escape quotes in keys and values
        xml_tag_escaped = xml_tag.replace('"', '\\"')
        class_name_escaped = class_name.replace('"', '\\"')
        code += f'    "{xml_tag_escaped}": "{class_name_escaped}",\n'
    code += "}\n\n"

    # Generate CLASS_IMPORT_PATHS dictionary
    code += "# Class name to import path mappings\n"
    code += "CLASS_IMPORT_PATHS: Dict[str, str] = {\n"
    for class_name, import_path in sorted(class_import_paths.items()):
        class_name_escaped = class_name.replace('"', '\\"')
        import_path_escaped = import_path.replace('"', '\\"')
        code += f'    "{class_name_escaped}": "{import_path_escaped}",\n'
    code += "}\n\n"

    # Generate POLYMORPHIC_TYPES dictionary
    code += "# Polymorphic type mappings (base type -> list of implementations)\n"
    code += "POLYMORPHIC_TYPES: Dict[str, List[str]] = {\n"
    for base_type, implementations in sorted(polymorphic_types.items()):
        base_type_escaped = base_type.replace('"', '\\"')
        impl_list = ", ".join(f'"{impl.replace(chr(34), chr(92) + chr(34))}"' for impl in implementations)
        code += f'    "{base_type_escaped}": [{impl_list}],\n'
    code += "}\n"
