...
class EthTpConnection(TpConnection):
    ...
    _XML_TAG = "ETH-TP-CONNECTION"


    tp_sdu_refs: list[ARRef]
    _DESERIALIZE_DISPATCH = {
        "TP-SDU-REFS": lambda obj, elem: [obj.tp_sdu_refs.append(ARRef.deserialize(item_elem)) for item_elem in elem],
    }
        ...
        for child in element:
            ...
            if tag == "TP-SDU-REFS":
                ...
                for item_elem in child:
                    obj.tp_sdu_refs.append(ARRef.deserialize(item_elem))

        return obj



class EthTpConnectionBuilder(TpConnectionBuilder):
    """Builder for EthTpConnection with fluent API."""
    ...
    def with_tp_sdus(self, items: list[PduTriggering]) -> "EthTpConnectionBuilder":
        """Set tp_sdus list attribute.
        ...
        """
        self._obj.tp_sdus = list(items) if items else []
        return self


    def add_tp_sdu(self, item: PduTriggering) -> "EthTpConnectionBuilder":
        """Add a single item to tp_sdus list.
        ...
        Returns:
            self for method chaining
        """
        self._obj.tp_sdus.append(item)
        return self

    def clear_tp_sdus(self) -> "EthTpConnectionBuilder":
        """Clear all items from tp_sdus list.
    ...
    def build(self) -> EthTpConnection:
        """Build and return the EthTpConnection instance with validation."""
